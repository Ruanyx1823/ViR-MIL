...
from torch.utils.data.sampler import WeightedRandomSampler, RandomSampler, SequentialSampler, SubsetRandomSampler
...
class SubsetSequentialSampler(sampler.Sampler):
...
def collate_tranformer(batch):
    data_s = torch.cat([item[0] for item in batch], dim = 0)
    coords_s = np.vstack([item[1] for item in batch])
    data_l = torch.cat([item[2] for item in batch], dim = 0)
    coords_l = np.vstack([item[3] for item in batch])
    label = torch.LongTensor([item[4] for item in batch])
    return [data_s, coords_s, data_l, coords_l, label]

def collate_body_part_aware(batch):
    """身体部位感知的数据整理函数"""
    data_s = torch.cat([item[0] for item in batch], dim = 0)
    coords_s = np.vstack([item[1] for item in batch])
    data_l = torch.cat([item[2] for item in batch], dim = 0)
    ...
    # 处理身体部位信息
    if len(batch[0]) > 5:  # 包含身体部位信息
        body_parts = [item[5] for item in batch]
        return [data_s, coords_s, data_l, coords_l, label, body_parts]
    else:
        return [data_s, coords_s, data_l, coords_l, label]
...
def get_simple_loader(dataset, batch_size=1, num_workers=1, mode='clam'):
    ...
    elif(mode == 'body_part_aware'):
        collate = collate_body_part_aware
    else:
        collate = collate_MIL
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='clam', num_workers=4):
    """
