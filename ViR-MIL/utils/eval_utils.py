...
def eval(mode, dataset, args, ckpt_path):
    ...
    patient_results, test_error, auc, test_f1, df, acc_logger, sensitivity, specificity = summary(mode, model, loader, args)
    ...
    return model, patient_results, test_error, auc, test_f1, df, each_class_acc, sensitivity, specificity
...
def summary(mode, model, loader, args):
    ...
    if(mode == 'transformer'):
        for batch_idx, (data_s, coord_s, data_l, coord_l, label) in enumerate(loader):
            ...
            with torch.no_grad():
                outputs = model(data_s, coord_s, data_l, coord_l, label)
                Y_prob, Y_hat, loss = outputs[:3]
                A_low = outputs[3] if len(outputs) > 3 else None
                A_high = outputs[4] if len(outputs) > 4 else None
            ...
            if A_low is not None:
                result_entry['A_low'] = A_low.squeeze(0).cpu().numpy()
            if A_high is not None:
                result_entry['A_high'] = A_high.squeeze(0).cpu().numpy()
            patient_results.update({slide_id: result_entry})
        ...
        if args.n_classes == 2:
            ...
            if cm.shape == (2, 2):
                ...
                sensitivity = tp / (tp + fn) if (tp + fn) > 0 else 0  # 敏感性 = 真阳性率
        ...
        return patient_results, test_error, auc_score, test_f1, df, acc_logger, sensitivity, specificity 
