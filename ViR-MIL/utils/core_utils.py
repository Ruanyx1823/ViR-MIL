...
# ---- 安全转换工具：兼容numpy/标量，统一转为Tensor并放到device ----
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, ckpt_name):
        ...
        try:
            ...
            print(f'Model checkpoint saved to: {ckpt_name}')
...
def train_loop(args, epoch, model, loader, optimizer, n_classes, writer = None, loss_fn = None):
    ...
    for batch_idx, (data_s, coord_s, data_l, coords_l, label) in enumerate(loader):
        # 兼容 numpy / list / scalar，统一转为Tensor并送至device
        data_s = to_device_tensor(data_s, device, torch.float32)
        coord_s = to_device_tensor(coord_s, device)
        data_l = to_device_tensor(data_l, device, torch.float32)
        coords_l = to_device_tensor(coords_l, device)
        label = to_device_tensor(label, device, torch.long)
        outputs = model(data_s, coord_s, data_l, coords_l, label)
        _, Y_hat, loss = outputs[:3]
...
def validate(cur, epoch, model, loader, n_classes, early_stopping = None, writer = None, loss_fn = None, results_dir=None):
    ...
    val_loss /= len(loader)
    # 将张量列表转换为NumPy数组
    all_label_np = torch.cat(all_label).cpu().numpy()
    all_pred_np = torch.cat(all_pred).cpu().numpy()
    val_f1 = f1_score(all_label_np, all_pred_np, average='macro')
